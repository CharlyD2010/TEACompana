...
import { Star, ChevronLeft, Loader2, Inbox, Rainbow, LayoutDashboard, Users, LogOut, Settings } from 'lucide-react';
...
export const AppButton = ({ className, variant, ...props }: React.ComponentProps<typeof Button>) => (
  <Button 
    variant={variant}
    className={cn(
      "rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all active:scale-95 shadow-sm h-12", 
  ...
  return (
    <div className="flex items-center justify-between p-4 md:p-6 bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50 border-b border-muted/50">
      <div className="flex items-center gap-4">
        {showBack && (
          <Button variant="ghost" size="icon" onClick={() => router.back()} className="rounded-full bg-muted/30 hover:bg-muted/50">
            <ChevronLeft className="h-5 w-5" />
          </Button>
        )}
        {!showBack && <div className="p-2 bg-primary/10 rounded-xl"><Rainbow className="h-6 w-6 text-primary" /></div>}
        <div className="flex flex-col">
          <h1 className="text-lg md:text-xl font-black text-primary uppercase tracking-tighter leading-none">{title}</h1>
          <div className="flex gap-3 mt-1.5">
            {showBackToChildren && (
              <button onClick={() => router.push('/children')} className="flex items-center gap-1 text-[9px] font-black text-muted-foreground uppercase hover:text-primary transition-colors tracking-widest">
                <Users className="w-3 h-3" /> Mis Niños
              </button>
            )}
            {showBackToDashboard && childId && (
              <button onClick={() => router.push(`/child/${childId}/dashboard`)} className="flex items-center gap-1 text-[9px] font-black text-muted-foreground uppercase hover:text-primary transition-colors tracking-widest">
